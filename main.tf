...
# ==== All remaining modules – no semicolons, perfect syntax ====
module "eks" {
  source             = "./modules/eks"
...
module "cloudfront" {
  ...
  origin_domain = module.s3_bucket.bucket_name
}

module "route53" {
  source      = "./modules/route53"
  create_zone = false
  zone_id     = "Z09999999999999999999"  # change later or set to real zone
  record_name = "test"
  record_type = "A"
  records     = ["1.2.1.1"]
}
...
module "cloudwatch_log_group" {
  ...
  log_group_name = "/aws/${var.project_name}"
}
...
module "secretsmanager" {
  ...
  secret_name  = "/${var.project_name}/db"
...
module "autoscaling_schedule" {
  source           = "./modules/autoscaling-schedule"
...
module "s3_replication" {
  ...
  replication_role_arn  = module.iam_role.role_arn
}

module "cloudwatch_dashboard" {
  source         = "./modules/cloudwatch-dashboard"
  dashboard_name = "${var.project_name}-dash"
  dashboard_json = jsonencode({ widgets = [] })
}
...
module "s3_logging" {
  ...
  target_bucket = module.s3_bucket.bucket_name
  log_bucket    = "${var.project_name}-logs-${random_string.suffix.result}"
}
